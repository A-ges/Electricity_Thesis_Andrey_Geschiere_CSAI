...
The results indicate that behavioral architecture is the dominant driver of individual-level outcomes, with price-responsive agents shifting approximately four times more per day than habit-driven agents. At the system level, population composition explained over 83\% of rank variance across all metrics. These findings suggest that demand response effectiveness depends thoroughly on population composition as well as individual flexibility.
...
This thesis presented an agent-based model for studying how heterogeneous household behavior shapes residential demand within a variable-priced electricity system. The framework simulates how household schedules and electricity prices evolve over a 30 day period. It combines three types of behaviors based on: habits, price responses, and social influences. These behaviors are exposed to a detailed social network, a calibrated baseline for appliance based energy use and a dynamic pricing system that reacts to demand.
 
In response to RQ1, the simulation indicated that behavioral architecture was a substantial driver of individual flexibility and adjustment within this model, with near-complete rank separation observed across all four plausible compositions. Under the chosen parameterization, price-responsive agents shifted approximately four times more per day than habit-driven agents and achieved the lowest normalized cost, while socially-influenced agents occupied an intermediate position whose cost benefit varied with the surrounding behavioral mix.
 
In response to RQ2, population composition drove every reported system metric within the model, explaining over 83\% of rank variance in each. The Price-Maximalist composition produced the largest aggregate load deformation and the most favorable price advantage. Compositions with a smaller price-responsive share, most notably Tipping, produced minimal deformation despite their elevated social-influenced proportions This suggests that socially open agents in this model require sufficient exposure to price-responsive peers to shift in a meaningful way. Network size did not meaningfully affect any behavioral or cost-related metric, indicating that the composition effects observed here are stable across the tested population scales.
