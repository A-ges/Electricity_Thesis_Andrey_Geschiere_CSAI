...
Each appliance is assigned a 24-hour baseline probability distribution, which represents the likelihood of the appliance being switched on during any given hour. This baseline consists of normalized per-appliance hourly switch-on data from \textcite{yilmaz_occupant_2017}. These distributions are then transformed into a maximum of four Gaussian peaks per appliance, represented as [peak\_hour, peak\_magnitude, peak\_width], by fitting sums of Gaussians to the switch-on data. The number of Gaussian peaks per appliance were selected using the Bayesian Information Criterion \autocite{schwarz_estimating_1978}. The peaks were then fitted with L-BFGS-B minimizer from the Scipy package \autocite{virtanen_scipy_2020}.
...
Peak solar hours near the lowest elasticity of $0.1$, suppressing price swings during RES surplus, while low-generation hours approach $0.6$. The linear transformation preserves relative spacing across hours regardless of these bounds.
...
where higher values indicate greater temporal concentration of demand and higher grid stress \autocite{makhadmeh_optimization_2019}. PAR is complemented with the peak hour, which provides directional context to PAR magnitudes by indicating whether peaks occur during high- or low-supply periods.
...
\begin{figure}
  ...
  \caption{\makebox[\textwidth][l]{Change of system metric across $N$ per population.}} 
  \label{fig:rq2n}
\end{figure}
